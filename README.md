...
- **高度开放**: 完全开源，免费使用，无任何广告植入，可自行部署和定制。
...
有关配置文件的编写格式和参数说明，请参考下面的「[管理订阅](#-管理订阅)」部分。

# 📖 管理订阅

为了更灵活的管理视频源等配置，飞鱼支持通过订阅来分享和导入配置文件。

你可以参考本地的 `feiyu.example.json` 文件，然后根据下面的参数说明配置自己的订阅。
...
为解决这个跨域问题，飞鱼提供了一个专用的请求代理服务，具体使用方法请参阅「[飞鱼 Proxy](#飞鱼-proxy)」部分。
...
飞鱼的初衷是**让每个人都能通过电影的力量，获得对生活的新体验和新认知，从而怀着永不熄灭的热忱，勇敢地去追求、去探索、去发现生命的意义和价值。**
